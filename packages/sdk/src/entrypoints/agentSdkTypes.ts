 ...
 */
/** Open Agent–specific options not present on the official SDK `Options` type. */
type OptionsWithOpenAgentExtras = Options & {
  apiKey?: string
  baseURL?: string
...
export function query(params?: any): any {
  ...
  const { prompt, options: rawOptions = {} } = params
  const options = rawOptions as OptionsWithOpenAgentExtras
